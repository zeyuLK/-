...
# 你自编的包
sys.path.append(r"D:\\Study\\RMPF\\scene")
from scene.loss import MultiView_all_loss
from scene.prepare import Multi_view_dynamic_relation
from scene.scene_data import *
...
def test(net):
    ...
    with torch.no_grad():  # 禁用梯度计算，提高推理效率
        for data in testloader:
            inputs_x1, inputs_x2, inputs_x3,label_train = data
            ...
            if use_cuda:
                inputs_x1, inputs_x2, inputs_x3, label_train,s_x1,s_x2,s_x3= inputs_x1.cuda(), inputs_x2.cuda(), inputs_x3.cuda(), label_train.cuda(non_blocking=True), s_x1.cuda(),s_x2.cuda(),s_x3.cuda()
                net = net.cuda()
            m_s_x1,m_s_x2,m_s_x3,\
               ...
               x_view_relation,adj_label,adj_recover= net(s_x1,s_x2,s_x3,inputs_x1, inputs_x2, inputs_x3, 3)
    ...
    x_view_final = (torch.cat(x1_feature).cpu().numpy() +
                    ...
                    torch.cat(x3_feature).cpu().numpy() ) / 3
...
if __name__ == "__main__":
    ...
    sectrained_model_path = 'D:/Study/RMPF/model_inf/scene15_second_trained_model.pth'
    try:
