...
def nmi(y_ture,y_pred):
    nmi_score = normalized_mutual_info_score(y_ture,y_pred)
    return nmi_score
...
def get_avg_acc(y_true, y_pred, count):
    ...
    for i in range(count):
        acc_array[i] = acc(y_true, y_pred[i])
...
def get_avg_nmi(y_true, y_pred, count):
    ...
    for i in range(count):
        nmi_array[i] = nmi(y_true, y_pred[i])
...
def get_avg_RI(y_true, y_pred, count):
    ...
    for i in range(count):
        RI_array[i] = randIndex(y_true, y_pred[i])
...
def get_avg_f1(y_true, y_pred, count):
    ...
    for i in range(count):
        f1_array[i] = f_score(y_true, y_pred[i])
